...
when isMainModule:
  ...
  for c in newInfos:
    if sourcesJson[c.rev] == nil:
       sourcesJson[c.rev] = newJArray()
    if sourcesJson[c.rev].kind == JObject:
       var tmp = newJArray()
       tmp.add(sourcesJson[c.rev])
       sourcesJson[c.rev] = tmp
    sourcesJson[c.rev].add(%* {"name": c.name, "url": c.url, "nixexprs_hash": c.nixexprs_hash, "programs_sqlite_hash": c.hash})
